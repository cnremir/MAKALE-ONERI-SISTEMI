...
user_interests = ["military", "mobile", "finance"]
...
#def anasayfa():
 ...
 #   session['user_interests'] = user_interests
   ...
   # return render_template('anasayfa.html', fasttext_recommendations=fasttext_recommendations, scibert_recommendations=scibert_recommendations)

 #   user_id = session.get('user_id')
  #  if user_id:
   #     user = collection.find_one({"_id": ObjectId(user_id)})
    #    if user:
     #       return render_template("anasayfa.html", user=user,fasttextList = fasttextList)  
...
def sil():
    ...
    if user_id:
        ...
        # İlgili kullanıcının ilgi alanları belgesini güncelle ve ilgi alanını listeden kaldır
        collection.update_one(
            {"_id": ObjectId(user_id)},
